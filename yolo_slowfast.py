...
from pytorchvideo.models.hub import slowfast_32x2_r101_50_50_detection
...
def main(config):
    ...
    model = torch.hub.load(r'E:\gitcode\yolov5-master', 'custom',path=r'E:\gitcode\yolov5-master\weights\best.pt',source='local')
    ...
    #video_model = torch.hub.load(r'E:\gitcode\yolo_slowfast', 'custom',path = r'E:\gitcode\yolo_slowfast\checkpoints\yz_r101.pyth',source='local')
    #video_model = video_model(True).eval().to(device)
    #video_model = slowfast_yz_r101_50_50(True).eval().to(device)

    #video_model = slowfast_16x8_r101_50_50(True).eval().to(device)
    #video_model = slowfast_r101(True).eval().to(device)
    #video_model = slowfast_r50_detection(True).eval().to(device)
    video_model = slowfast_32x2_r101_50_50_detection(False).eval().to(device)
    deepsort_tracker = DeepSort("deep_sort/deep_sort/deep/checkpoint/ckpt.t7")
    #deepsort_tracker = DeepSort("deep_sort/deep_sort/deep/checkpoint/osnet_x0_25_msmt17.pth")
    ava_labelnames,_ = AvaLabeledVideoFramePaths.read_label_map("selfutils/temp.pbtxt")
    coco_color_map = [[random.randint(0, 255) for _ in range(3)] for _ in range(80)]

    vide_save_path = config.output

    input_video = os.listdir(config.input)

    for video_name in input_video:
        video = cv2.VideoCapture(os.path.join(config.input,video_name))
        #print(video_name)

        width,height = int(video.get(3)),int(video.get(4))
        video.release()

        outputvideo = cv2.VideoWriter(os.path.join(vide_save_path,video_name),cv2.VideoWriter_fourcc(*'mp4v'), 25, (width,height),True)
        print("processing...")

        video = pytorchvideo.data.encoded_video.EncodedVideo.from_path(os.path.join(config.input,video_name))
        a=time.time()
        flag = 1
        for i in range(0,math.ceil(video.duration),1):
            #print(math.ceil(video.duration))
            video_clips=video.get_clip(i, i+1)
            #这里读出来的是一秒钟获得的video_clips数量 使用这个video_clips在yolov5上进行目标检测跟踪
            if flag == 1:
                print("video_clips is %d", video_clips['video'].shape)
                flag = 0
            print("------------------")
